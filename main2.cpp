...
static void setupRoad(){
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * road.getNumCurves()* road.getNumVertecies() * 3, &vertexPositionData[0], GL_STATIC_DRAW);
...
static void createGeomentry(void) {
  ...
  glGenBuffers(1, &platformPositionVBO);
  glBindBuffer(GL_ARRAY_BUFFER,platformPositionVBO);
  glBufferData(GL_ARRAY_BUFFER, numVertices * sizeof(Vector3), vertexPositions, GL_STATIC_DRAW);


  glGenBuffers(1, &platformNormalVBO);
  glBindBuffer(GL_ARRAY_BUFFER,platformNormalVBO);
