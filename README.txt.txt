Advanced Computer Graphics Final Project

Group Members: 
	Miral Panchal
	Muhammad Faizan Ali
	Shubham Patel

Project Summary

- This project is a car animation driving on a path 
- path is drawn using bazier curves made of multiple bezier curves also possible to draw a road on the curve, but didn't look nice
- multiple light sources have been applied
- bats in the sky acting as BOIDS
- Skybox added using texture mapping.

Note**
- To zoom in and out hold right click and move cursor.
- To move camera hold left click and move cursor.

Licensing for Ice River Skybox textures

Author
======

This is the work of Emil Persson, aka Humus.
http://www.humus.name



License
=======

This work is licensed under a Creative Commons Attribution 3.0 Unported License.
http://creativecommons.org/licenses/by/3.0/
